...
class TestForecastEndpoint(unittest.TestCase):
    ...
    def test_cost_cum_deltas_sum_to_mtd(self):
        # Single-pass invariant: month_to_date must equal the sum of the per-day
        # cost_cum increments (within the 2-vs-4 dp rounding gap). This locks in the
        # "one query, MTD == sum(cost_cum deltas)" claim after folding cost_between +
        # cost_cum_series into one pass.
        now = int(time.time())
        lt = time.localtime(now)
        month_start = int(time.mktime((lt.tm_year, lt.tm_mon, 1, 0, 0, 0, 0, 0, -1)))
        with app.LOCK:
        ...
        cc = cm["cost_cum"]
        self.assertGreater(len(cc), 0)
        # cost_cum is the running total per day; its final element is the cumulative
        # spend, which must equal month_to_date within rounding (2dp vs 4dp).
        self.assertAlmostEqual(cc[-1], cm["month_to_date"], delta=0.01)
        # And the deltas between successive cumulative points sum back to the final
        # cumulative value (a self-consistency check on the per-day bucketing).
        deltas = [cc[0]] + [cc[i] - cc[i - 1] for i in range(1, len(cc))]
        self.assertAlmostEqual(sum(deltas), cc[-1], delta=0.01)
