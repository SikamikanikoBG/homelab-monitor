...
**Added**
- **Memory-junction temperature as a per-card metric on the GPU tab.** The core temperature was the only thermal number the tab charted per card, while the VRAM dies — a separate sensor, and on GDDR6X routinely 20-30 °C hotter than the core — appeared only as one pooled "Mem temp" chip showing the hottest card in the box. `temp_mem` has been *collected* per card (nvidia-smi `temperature.memory`, amdgpu sysfs) and stored per card per poll all along; it simply never left the database. It now has: a **Mem temp** row in each card's small-multiples panel, sitting directly under Temp so the two sensors on one card read as a pair; its own entry in the by-metric chart picker, with the memory-junction limit drawn as the threshold line instead of the core `hot_c`; a **Peak mem** column in Card health, so a range answers "did this card's memory ever cook" and not just "how hot did the core get"; and live folding, so the row tracks at the live-refresh cadence like every other per-card number. Read against its own thresholds (amber from 85 °C, red from 95 °C) rather than the configured core threshold — those come from the memory's own specification, not from a knob about core temperature. Where no card in the box has the sensor the row, the metric button and the health column are absent altogether rather than showing a permanent "not reported" line — worth knowing which hardware that is: **NVIDIA only exposes `temperature.memory` on datacentre cards** (A100/H100-class HBM), so every GeForce and Quadro box answers `[N/A]`, NVML's `FI_DEV_MEMORY_TEMP` field included — verified on a 3090 and a P2000 against driver 580.159. AMD cards report it through amdgpu's hwmon and light the row up; consumer NVIDIA cards will not, and no amount of asking the driver differently changes that.
- **Memory temperature now survives the hourly downsample, as a peak.** `gpu_samples_1h` gains `temp_mem`/`temp_mem_max`/`temp_mem_cnt`, so ranges past the raw-retention window (7d/30d/all) keep the memory row instead of going blank there the way Mem-BW and Clock do — and they keep the hour's **MAX**, not just its average. That distinction is the whole point for VRAM: memory-junction heat is what trips a sticky slowdown a card doesn't recover from until it's properly cooled, and an hourly average smooths precisely that event away. Averaged over the polls that actually reported the sensor (`temp_mem_cnt`), the same rule the fan average follows, so an intermittently-answering sensor doesn't get dragged toward zero.
