...
**Changed**
- **The Overview fits the fold — on any screen, without clipping anything.** The fleet rail used to hide its overflow behind a 560px scroll box, which cut the last host card in half, and the rest of the board simply ran past the bottom of the window. Nothing clips now, and the cockpit measures how far it overruns the screen after each paint and gives ground in order: the event feed and the cost list trim (each says how many rows it left out and links to its tab), the fleet rail switches to a tight tile that keeps the micro-bars but drops the OS line and the footer, the gauges and the burn figure lose their ceremony, and last of all the Launchpad folds pins behind a **+N more** tile — never below five, and back on one click. Only if all of that still overruns does it scale, by at most 22%, with the layout height compensated so the page genuinely stops scrolling. Verified from 4K down to 1366×768. A phone still stacks and scrolls, as it should.
- **Display mode fills the screen.** It used to letterboard a 1440px board in the middle of the monitor and scale it; now the board is fluid, the grid takes the leftover height, and every panel and list stretches into its share of it — full width, full height, no transform.
...
## [0.35.0](https://github.com/SikamikanikoBG/homelab-monitor/releases/tag/v0.35.0) — 2026-09-19 · **Your homelab gets a front page — and a wall screen**
**Added**
- **A Launchpad on the Overview — the apps you actually open, pinned.** The dashboard was a monitor; the tab you leave open all day wants to be a homepage too. The Overview's centre column now leads with a grid of shortcuts: Immich, the music server, the chess game, the router — whatever you keep typing a port number for. Pin one from any container row on the Containers tab (★, one click, the address pre-filled from the published port) or with **+ Pin** for anything with a URL, group them (Media / AI / Admin, or your own labels), drag to reorder while **Edit** is on. Each tile's icon is the app's real logo where we recognise it (the ~70 brand marks the dashboard already ships), one of 28 stroke icons from the dashboard's own family, or the name's initials on a stable colour — never a mismatched sticker. **A tile pinned from a container carries that container's actual state as its dot** (running / unhealthy / gone / that host is offline); a tile that names no container gets no dot at all, because a green light nobody verified is worse than none. Pins live on the hub (the `home_shortcuts` setting, `GET`/`POST /api/shortcuts`), not in the browser: pin it on the laptop and it is there on the phone and on the wall screen, and it rides the existing settings backup.
- **Display mode — the dashboard as a wall screen.** A **⛶ Display** button next to Share (or `/?display=1`, which is all a dedicated kiosk box needs) drops every scrap of chrome and scales the whole board with one transform so it **fits the screen exactly, with no scrolling** — at any size, on any monitor. A clock in the corner, the cursor hides after five seconds, `Esc` leaves. You choose which panels the wall shows (Launchpad / Fleet / Burn rate / Engine / Event feed / Costs — costs off by default: a long list reads badly across a room), and on Chromium-based browsers it enumerates your monitors and opens full-screen on the one you pick; elsewhere it uses the current screen and says so rather than pretending.
- **A Clock (MHz) row in each GPU card's chip stack.** The backend has tracked per-card core clock history (`clk_sm`) since the TFLOPS-at-current-clock feature shipped — `/api/gpu/history` already carries the series and a `supports` flag per card — but the small-multiples view on the GPU tab never listed it as one of its rows. It now sits alongside Util/TFLOPS/VRAM/Temp/Fan/Power, same sparkline-and-value treatment, and reads "not reported" rather than a flat line on a driver that won't report it.
- **A live-refresh pulse next to the MCP status.** A small dot rings once every time the tab you're actually looking at lands a real refresh — the Overview cockpit, GPU, Host, Network, Security, Disk I/O, Costs, Experiments, Benchmarks, Models and Containers/Services tabs each trigger it from their own genuine data landing, not a fixed decorative timer. A remote host's GPU tab (refreshed only every ~15-60s) and the Overview cockpit (refreshed every couple of seconds) both ring at their own true rate, so the dot is always telling the truth about how often what's on screen actually updated.
...
**Changed**
- **The Overview is a front page again, not an index of everything.** The compute-history chart, the containers / services / diagnostics strip and the collapsed *AI workload · MCP · setup* block are gone from it — every number in them has its own tab one click away, and each was a second place to maintain the same truth. What is left is the Launchpad, Fleet, Burn rate, the Lab's Engine, the Event & Insight feed and What's costing you. **Setup & requirements** moved to the **System** tab (the setup banner links there now), and the **AI agent (MCP)** card became a popover on the header's MCP pill — one click from the status that was already sitting there, instead of buried in a `<details>` at the bottom of a page nobody scrolled. The dead drag-board code went with them; tapping a panel header still jumps to that panel's tab.

**Fixed**
- **The AI Models tab: custom servers stopped vanishing, and the panels stopped jumping.** Four distinct bugs, all of which made a correctly-registered vLLM look broken. (1) A custom server registered for a **remote** box had its models listed on the **hub's** panel, because only the registry rows carried the fleet host — the live rows (`/api/data`, `/api/ai/now`) did not. Every model row now carries the host it belongs to, and each panel shows only its own. (2) A custom server whose probe came back empty **disappeared from the tab entirely** — indistinguishable from "never registered", which is exactly what made the feature feel unreliable: you registered it, Test passed, and then nothing. A server that stops answering now says so on the host it was registered for, with its address and why. (3) The server cards were rebuilt from scratch on every 5-second poll and re-sorted into whatever order the probes answered in, so tooltips died mid-hover, text selection dropped and cards **swapped places while you read them**; they are keyed by name, sorted stably and patched part by part now. (4) A remote box serving from a non-Ollama server showed **"0 MB VRAM committed"** under a model it was visibly running (only Ollama reports per-model VRAM, and the hub's `nvidia-smi` cannot see another box's processes) — it now shows that host's own VRAM-in-use figure, labelled as not attributable per model, and a model confirmed live by its own `/metrics` says **serving** instead of a dash next to an empty bar.
- **Installed models missed every ollama the hub's registry could not reach.** The on-disk inventory only ever talks to one ollama (`COPILOT_OLLAMA_URL`), and every other hub-local ollama entry was dropped as a duplicate of it — so a second ollama container, or the only one listening on a non-default port, had all of its models missing from the Installed-models list while the panel above it showed them loaded. They are deduplicated by name now instead of discarded wholesale. The summary line also stops claiming *0,00 GB on disk* when no server reports a size.
- **`/api/models` returned 500 for any fleet with a Windows host that had models ([#292](https://github.com/SikamikanikoBG/homelab-monitor/issues/292), reported by @jbradf0rd).** `probe.ps1` wrapped its model array twice (`return ,$out` under a caller's `@(…)`), so a Windows host shipped `[[…]]` and the hub raised `ValueError: dictionary update sequence element #0 has length 11; 2 is required` — the whole AI Models tab sat on a spinner. The probe no longer double-wraps, and the hub unwraps a nested remote catalogue defensively so an older probe on a remote box can never take the endpoint down again.
- **A light that comes on when a host's GPUs stop being watched.** A driver auto-update on a GPU box left `nvidia-smi` dying with *Driver/library version mismatch*, the probe reported "no GPU", and the dashboard sat on the last numbers it had for a day without a single red pixel — the owner noticed only because the temperature didn't move under load. Three things now happen instead. The probe (Linux `probe.py` and the hub's own sampler) tells *"no GPU"* apart from *"GPU tool broken"* — it checks for a loaded NVIDIA kernel driver (`/proc/driver/nvidia`, `/dev/nvidiactl`) and reports the tool's own first line as `gpu_error` (a driver/library mismatch, a hung tool, a driver without `nvidia-smi` installed). The hub remembers which hosts had cards and gives every host a verdict (`gpu_telemetry` on `/api/fleet`, `telemetry` on `/api/gpu/history`), and every host-level surface shows it: a pulsing red dot on the host pill in the header, a **GPU LOST** badge on the Overview host tile, *GPU telemetry lost* in the fleet table's GPU column, and on the GPU tab a red strip quoting the error, a **NO TELEMETRY** pill per card, dashes instead of zeros in the KPIs and every last-known number marked as such. And the notifier raises **one** host-level critical — *"{host}: GPU telemetry lost"*, with the error text and how many cards it was reporting until when — after the loss has persisted for two minutes; it clears when the cards return, and after an hour a host whose cards were removed on purpose stops being nagged. Previously a host whose cards *all* vanished at once was dropped from the alert scan entirely, so the existing "GPU N stopped reporting" alert only ever fired for a partial loss.
- **Every remote host refreshed at the pace of the slowest one.** The host poller ran one pool per cycle and slept `SAMPLE_INTERVAL` only after the whole cycle finished, so every host's period was *(slowest probe + interval)*: a Windows box whose PowerShell probe takes 40 s made a Linux GPU box that answers in 4 s refresh every ~50 s, and the GPU tab looked like it ignored the configured refresh interval. Each host now rides its own clock — a fast host is re-polled every interval, a slow one as soon as its previous probe lands, and neither waits on the other. The Live refresh interval caption in Settings says plainly that remotes move at the sample interval.
- **The same up-to-60s lag the GPU tab had was on Network, Security, a remote Host tab, Containers, Services, and Disk I/O too.** All of them rode the generic history timer, throttled to whichever chart's bucket size happens to be (clamped to [15s,60s]) — a throttle that makes sense for a chart, not for `/api/health` or a remote host's SSH-polled data, both of which already refresh server-side every ~10s regardless of what the browser asks for. They now share a dedicated 5s poll, decoupled from the chart-bucket clamp. Costs, Experiments and Benchmarks already had their own internal throttles gating ahead of anything user-visible, so they didn't need this. The live-refresh pulse on each of these was also ringing on every poll rather than every genuine change — fixed the same way as the GPU tab: it now compares each fetch's data against the last one and only rings when something actually moved.
- **A remote host's GPU tab waited up to 60s to show numbers its own server-side poll had ready in 10.** The tab rode the same generic history timer every long-range chart uses, throttled to the chart's bucket size and clamped to [15s,60s] — sensible for a chart, but a remote host's GPU stats are already refreshed every ~10s by the SSH-based host poller regardless, so the extra wait was pure added lag on top of data already sitting there. A dedicated 5s poll now closes that gap for whichever host is selected; it does not (and structurally cannot, without a redesign of the SSH round-trip that fetches remote GPU stats) make a remote host's own numbers arrive faster than that poller's own cadence — the live-refresh pulse now genuinely reflects that, ringing only when the underlying numbers actually moved rather than whenever a poll happened to land.
- **Fan speed on the GPU tab only ever moved on the full 10s sample, however fast the live refresh was set.** `gpu_cards_fast()`'s own docstring has always promised util/VRAM/power/temp **and fan**, but the nvidia-smi query behind it never actually asked for `fan.speed` — so of the five numbers on a GPU card, four moved at the fast-lane cadence and fan quietly waited out the full sampler regardless. The frontend was already wired to merge a live fan reading in (`mergeGpuTail`); it just never got one between full samples. Same request-with-fan/fall-back-without-it pattern the full sampler already uses for drivers too old to recognise the field.
- **A remote host's GPU tab could freeze until a hard reload.** `renderGpuTab()`'s two fetches (`/api/gpu/history` + `/api/gpu/attribution`) had no timeout, so a request that never settles — a backgrounded tab the browser suspended and later woke, a dropped connection it never resets — left the tab's "fetch in flight" guard permanently set, silently swallowing every later poll for that host until the page was reloaded and its JS state started fresh. Both fetches now abort after 15s, generous for a same-LAN request, so a stuck one clears itself and the next scheduled poll can retry instead of the tab going quiet forever.
